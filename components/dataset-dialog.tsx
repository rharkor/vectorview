...
export function DatasetDialog() {
  ...
  const [importOptions, setOptionsState] = useState<ImportOptions>(DEFAULT_IMPORT_OPTIONS);
  ...
  const selectedTable = tables?.find((t) => `${t.schema}.${t.name}` === selectedKey) ?? null;
  ...
  const fetchTables = async (term: string, autoSelect: boolean) => {
    ...
    if (autoSelect) {
      const first = body.tables.find((t: SourceTable) => hasMapping(t.suggestion));
  ...
  const hasMapping = (s: SourceTable["suggestion"]) => Boolean(s.idColumn && s.embeddingColumn);
    ...
    fetch("/api/dataset/count", {
      ...
      .then(async (res) => {
        ...
        if (res.ok && typeof body?.count === "number") {
          ...
          setEmbeddingCount(typeof body.embeddingCount === "number" ? body.embeddingCount : null);
        }
      })
      .catch(() => {})
      .finally(() => {
    ...
    try {
      const res = await fetch("/api/dataset/import", {
        ...
        body: JSON.stringify({
          ...
          clusterColumn: mapping.clusterColumn === NONE ? null : mapping.clusterColumn,
          labelColumn: mapping.labelColumn === NONE ? null : mapping.labelColumn,
      ...
      for (;;) {
        ...
        for (const chunk of events) {
          ...
          if (event.phase === "done") {
            ...
            toast.success(`Imported ${Number(event.rows ?? 0).toLocaleString()} rows`);
            bumpDataVersion();
            await refreshInfo();
          }
  ...
  const progressPercent = (p: ProgressEvent): number | null => {
    if (p.done !== undefined && p.total && p.total > 0) return (p.done / p.total) * 100;
    return null;
  };
  ...
  const progressLabel = (p: ProgressEvent): string => {
    ...
    if (p.phase === "projecting" && p.step) return STEP_LABELS[p.step] ?? p.step;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            The app stores its own copy of the data. Import from any Postgres database (connected
            read-only) or clear the current dataset.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-lg border border-border p-3 text-sm">
          {info && info.rowCount > 0 ? (
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="font-medium">
                  ...
                  {info.source?.embeddingDim ? ` · ${info.source.embeddingDim} dims` : ""}
                </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="text-red-300">Couldn&apos;t reach the dataset API: {infoError}</p>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
          ...
          <div className="grid gap-3">
            <div className="grid gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Read-only access is enforced at the connection level. The URL is remembered in this
                browser.
              </p>
            </div>
                      ...
                      <Command shouldFilter={false} className="flex w-full flex-col">
                        ...
                        {totalTables !== null && tables && totalTables > tables.length && (
                          <p className="border-b border-border px-3 py-1.5 text-[11px] text-muted-foreground">
                            Showing {tables.length} of {totalTables} tables — type to search the
                            rest
                          </p>
                        )}
                ...
                {selectedTable && (
                  <div className="grid grid-cols-2 gap-3">
                    <ColumnSelect
                      ...
                      onChange={(v) => setMapping((m) => ({ ...m, idColumn: v }))}
                    ...
                    <ColumnSelect
                      ...
                      onChange={(v) => setMapping((m) => ({ ...m, clusterColumn: v }))}
                      columns={columnOptions((c) => c.udt !== "vector")}
                      allowNone
                    />
                    <ColumnSelect
                      ...
                      onChange={(v) => setMapping((m) => ({ ...m, labelColumn: v }))}
                      columns={columnOptions((c) => c.udt !== "vector")}
                      allowNone
                    />
                    <div className="grid grid-cols-3 gap-1.5">
                      {(["x", "y", "z"] as const).map((axis) => (
                        <ColumnSelect
                          ...
                          value={mapping[`${axis}Column` as keyof typeof mapping]}
                          onChange={(v) => setMapping((m) => ({ ...m, [`${axis}Column`]: v }))}
                    ...
                    {mapping.xColumn !== NONE && mapping.yColumn !== NONE && (
                      <p className="col-span-2 text-xs text-muted-foreground">
                        Source coordinates detected — projection will be skipped.
                      </p>
                    )}
                  </div>
                )}
                  ...
                  <>
                    ...
                    {showMore && (
                      <div className="grid gap-3 rounded-lg border border-border p-3">
                        <p className="text-xs text-muted-foreground">
                          kubectl relay is still one tunnel. Keep a single connection and moderate
                          page size or the heartbeat dies mid-copy.
                        </p>
                        <div className="grid grid-cols-2 gap-3">
                          <NumberField
                            label="Rows per page"
                          ...
                          <NumberField
                            ...
                            min={1}
                            max={4}
                            onChange={(connections) => updateOptions({ connections })}
                          />
                          <NumberField
                            ...
                            min={15}
                            max={600}
                            onChange={(fetchTimeoutSec) => updateOptions({ fetchTimeoutSec })}
                          />
                          <NumberField
                            label="UMAP fit rows"
                            ...
                            min={500}
                            max={100000}
                            onChange={(umapFitMax) => updateOptions({ umapFitMax })}
                          />
                        </div>
                        <div className="flex items-center justify-between gap-3 text-sm">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(includePayload) => updateOptions({ includePayload })}
                          />
                        </div>
                        <div className="flex items-center justify-between gap-3 text-sm">
                          <div>
                          ...
                          <Switch
                            ...
                            onCheckedChange={(buildHnsw) => updateOptions({ buildHnsw })}
                          />
                        </div>
                      </div>
                    )}
...
function ColumnSelect({
  ...
  return (
    <div className="grid gap-1.5">
      <Label className="text-xs">{label}</Label>
      <Select value={value} onValueChange={(v) => onChange(v ?? (allowNone ? NONE : ""))}>
          ...
          {columns.map((c) => (
            <SelectItem key={c.name} value={c.name}>
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="shrink-0 text-xs text-muted-foreground">
                  {c.udt === "vector" && c.vectorDim ? `vector(${c.vectorDim})` : c.dataType}
                </span>
              </span>
            </SelectItem>
          ))}
